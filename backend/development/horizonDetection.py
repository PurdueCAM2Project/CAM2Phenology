...
def findHorizon(filepath, new_path):
#	sourceImages = os.listdir(filepath)
#	sourceImages.sort()
	#for image in sourceImages:
	numMaskPixels = 0
	originalImage = cv2.imread(filepath, 0) #load initial image in grayscale
	colorOriginal = cv2.imread(filepath)
	...
	cv2.imwrite(new_path, maskedGrayImage)
