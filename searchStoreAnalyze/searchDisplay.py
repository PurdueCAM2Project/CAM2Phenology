...
from phenImage import downloadImage
addedTypes=[]
addedParameters=[]
top=tkinter.Tk()
flickr=Search()			
...
def displayImage(i):
	path=flickr.package.getImagePath(i)
	img=Image.open(path)
	img=img.resize((960, 540), Image.ANTIALIAS)
	im=ImageTk.PhotoImage(img)
	panel.configure(image=im)
	panel.image=im
	index=imagenum%flickr.package.data['Total']
	packageInfoLabel.configure(text='Package: '+flickr.package.data['Name']+'Package Rating: '+str(flickr.package.data['Overall Rating'])+'\nPhoto Date: '+flickr.package.data['Photos'][index]['DateTaken'])
	packageInfoLabel.text='Package: '+flickr.package.data['Name']+'Package Rating: '+str(flickr.package.data['Overall Rating'])+'\nPhoto Date: '+flickr.package.data['Photos'][index]['DateTaken']
...
def rate():
	index=imagenum%flickr.package.data['Total']
	flickr.package.rate(rating[0], rating[1], rating[2], rating[3], index)
...
B9=Button(top, text='Save', width=10, command=flickr.package.writedata)
creator=Button(top, text='package creator', width=10, command=flickr.packageCreator)  #launches packageCreator in search.py to create or import package
b.grid(row=0, column=2)
b1.grid(row=1, column=2)
b2.grid(row=11, column=0)
b3.grid(row=27, column=13)
